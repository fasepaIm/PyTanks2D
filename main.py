...
BUFFS_SPAWN_EVENT = pygame.USEREVENT + 2  # событие для спавна баффов
AMMUNITION_EVENT = pygame.USEREVENT + 3   # событие для перезарядки
FUEL_EVENT = pygame.USEREVENT + 4         # событие для сжигания топлива
FUEL_SPAWN_EVENT = pygame.USEREVENT + 5   # событие для спавна топлива

# Добавляем таймеры
pygame.time.set_timer(ENEMIES_EVENT, 2000)  # добавляем таймер на событие возрождения врагов
pygame.time.set_timer(PARTICLE_EVENT, 70)  # добавляем таймер на частицы
pygame.time.set_timer(BUFFS_SPAWN_EVENT, 30000)  # добавляем таймер для спавна баффов
pygame.time.set_timer(AMMUNITION_EVENT, 1500)  # добавляем таймер для перезарядки
pygame.time.set_timer(FUEL_EVENT, 1000)  # добавляем таймер для уменьшения топлива
pygame.time.set_timer(FUEL_SPAWN_EVENT, 3000)  # добавляем таймер для спавна топлива
...
def play_movie(screen, clock):
    filename = path.join(videos_folder, "video.mp4")
    ...
    while running:
        for event in pygame.event.get():  # Обрабатываем события
            if event.type == pygame.QUIT:  # если выйти
                running = False  # цикл останавливается
                show_menu = False
        ret, img = cap.read()
        if not ret:
            running = False
...
# функция отрисовки полосок
def bar_drawing(surf, x, y, bar_length, bar_height, pct):
...
def game():
    ...
    Map = TiledMap(path.join(map_folder, 'map_1.tmx'))  # загружем карту
    ...
    breakable_walls = pygame.sprite.Group()  # Разрушаемые стены
    breakable_houses = pygame.sprite.Group()  # Разрушаемые дома
    ...
    repair_kits = pygame.sprite.Group()  # Ремкомплекты
    speed_up_buffs = pygame.sprite.Group()  # Баффы скорости
    clear_map_buffs = pygame.sprite.Group()  # Баффы зачистки карты
    power_bullet_buffs = pygame.sprite.Group()  # Баффы на бронебойный снаряд
    fuel_buffs = pygame.sprite.Group()  # Топливо

    for tile_object in Map.tmxdata.objects:  # проходим по всем объектам карты
        if tile_object.name == 'player':  # если имя объекта - "player"
            player = Player(tile_object.x, tile_object.y, boom_flash)  # то создаём игрока в полученных координатах
        ...
        if tile_object.name == 'breakable_wall':  # если имя объекта - "breakable_wall"
            BreakableWall(all_sprites, breakable_walls, (tile_object.x, tile_object.y))  # то добавляем разрушаемую стену
        if 'house' in tile_object.name:
            BreakableHouse(all_sprites, breakable_houses, (tile_object.x, tile_object.y), tile_object.name)
        if tile_object.name == 'buff':
            BUFF_SPAWN_COORDINATES.append((tile_object.x, tile_object.y))
        if tile_object.name == 'fuel':
            FuelBuff(all_sprites, fuel_buffs, (tile_object.x, tile_object.y))
            FUEL_SPAWN_COORDINATES.append((tile_object.x, tile_object.y))
    ...
    if NIGHT:  # если ночь
        ...
        lamp_image = pygame.image.load(LAMP_IMAGE).convert_alpha()  # загружаем лампочку индикации врагов

    fuel_lamp_image = pygame.image.load(FUEL_LAMP_IMAGE).convert_alpha()  # загружаем лампочку индикации
    ...
    while running:  # Основной цикл программы
        ...
        for event in pygame.event.get():  # Обрабатываем события
            if event.type == pygame.QUIT:  # если выйти
                ...
                if not paused and not game_over:  # если не поузы и не проигрыш
                    ...
                    if event.key in [K_w, K_a, K_s, K_d] and player.move_speed != 0:  # если нажата одна из клавиш WASD
                    ...
                    if event.key == K_SPACE:  # если нажали пробел
                        if AMMUNITION > 0:  # если боезапас больше нуля
                            Bullet(all_sprites, bullets, walls, breakable_walls, breakable_houses,
                                   player.rect.center, player, boom_flash)  # создаём снаряд
            ...
            if not paused and not game_over:
                if event.type == PARTICLE_EVENT and player.move_speed != 0:  # если событие частиц
                ...
                if event.type == BUFFS_SPAWN_EVENT and \
                        len(repair_kits) + len(speed_up_buffs) +\
                        len(clear_map_buffs) + len(power_bullet_buffs) < 1 and not paused:
                    # если событие спавна баффа и на поле меньше одного
                    # спавним бафф
                    coords = random.choice(BUFF_SPAWN_COORDINATES)
                    buff = random.choice(['speed_up_buff', 'repair_kit', 'clear_map_buff',
                                          'power_bullet_buff', 'fuel_buff'])
                    if buff == 'speed_up_buff':
                        SpeedUpBuff(all_sprites, speed_up_buffs, coords)
                    elif buff == 'clear_map_buff':
                        ClearMapBuff(all_sprites, clear_map_buffs, coords)
                    elif buff == 'power_bullet_buff':
                        PowerBulletBuff(all_sprites, power_bullet_buffs, coords)
                    elif buff == 'fuel_buff':
                        FuelBuff(all_sprites, fuel_buffs, coords)
                    else:
                        RepairKit(all_sprites, repair_kits, coords)

                if event.type == FUEL_EVENT and not paused:
                    player.current_fuel -= FUEL_CONSUMPTION

                if event.type == FUEL_SPAWN_EVENT and not paused:
                    fuel_points = [False] * len(FUEL_SPAWN_COORDINATES)
                    for buff in fuel_buffs:
                        for i in range(len(FUEL_SPAWN_COORDINATES)):
                            if buff.rect.center == FUEL_SPAWN_COORDINATES[i]:
                                fuel_points[i] = True
                    for point in range(len(fuel_points)):
                        if not fuel_points[point]:
                            FuelBuff(all_sprites, fuel_buffs, FUEL_SPAWN_COORDINATES[point])

        if key_state[K_a] and player.move_speed != 0:  # если зажата клавиша A
            left = True  # налево
            draw_particle = True  # рисуем частицы

        elif key_state[K_d] and player.move_speed != 0:  # если зажата клавиша D
            right = True  # направо
            draw_particle = True  # рисуем частицы

        elif key_state[K_w] and player.move_speed != 0:  # если зажата клавиша W
            up = True  # наверх
            draw_particle = True  # рисуем частицы

        elif key_state[K_s] and player.move_speed != 0:  # если зажата клавиша S
            down = True  # вниз
            draw_particle = True  # рисуем частицы
        ...
        if not paused and not game_over:
            ...
            player.update(left, right, up, down, all_sprites, enemies, walls, breakable_walls, breakable_houses,
                          repair_kits, speed_up_buffs, clear_map_buffs, power_bullet_buffs, 
                          fuel_buffs)  # обновляем позицию игрока
            ...
            enemies.update(ENEMY_SPEED, walls, breakable_walls, breakable_houses, bullets,
                           all_sprites, boom_flash, player, enemies)  # обновляем позиции врагов
            breakable_walls.update()
            ...
            if player.baff_works:
                bar_x, bar_y = center_coords
                bar_drawing(screen, bar_x - 25, bar_y - 30,
                            50, 10, player.baff_time_left / player.baff_time)  # рисуем остаток времени баффа
            if NIGHT:  # если ночь
                fog.render_fog()  # то отрисовываем туман
            text_print(screen, 10, 5, f'Health:', path.join(fonts_folder, '20219.ttf'), WHITE, 20)
            bar_drawing(screen, 10, 25, 100, 20, player.health / PLAYER_HEALTH)  # рисуем здоровье игрока
            text_print(screen, 10, 55, f'Fuel:', path.join(fonts_folder, '20219.ttf'), WHITE, 20)
            bar_drawing(screen, 10, 75, 100, 20, player.current_fuel / PLAYER_FUEL)  # рисуем запас топлива игрока
            # отрисовываем кол-во снарядов
            text_print(screen, 10, 100, f'Bullets: {AMMUNITION}', path.join(fonts_folder, '20219.ttf'), WHITE, 20)
            # отрисовываем счёт
            ...
            if any([i.lamp for i in enemies]) and NIGHT:
                screen.blit(lamp_image, (120, 0))
            # отрисовываем лампочку, если заканчивается топливо
            if player.current_fuel / PLAYER_FUEL <= 0.3:
                screen.blit(fuel_lamp_image, (130, 65))
